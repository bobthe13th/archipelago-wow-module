...
**Spell chains are real and usable.** `SpellChainNode` (`src/server/game/Spells/SpellMgr.h:550-557`, fields `prev`/`next`/`first`/`last`/`rank`) is populated by `SpellMgr::LoadSpellRanks` (`SpellMgr.cpp:~1280-1380`) from the real `spell_ranks` DB table (`first_spell_id, spell_id, rank` columns; corrected 2026-09-08 — an earlier pass of this research misnamed it `spell_chain`, which is only the in-memory map name, `mSpellChains`) and queryable via `SpellMgr::GetSpellChainNode(uint32 spell_id)` (`SpellMgr.h:674`). The same map also covers talent rank chains (`SpellMgr::LoadSpellTalentRanks`, driven by `Talent.dbc`) — trainer-taught ability ranks (Fireball R1-R7 etc.) come from `spell_ranks`, not talents. Rank-chain traversal is a solved problem — no new data source needed.
